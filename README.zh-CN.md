...
1. 一个精简 HTML 审阅页：包含 10 分钟决策仪表盘、P0/P1 立即处理清单、研究者必须确认的关键决策、关键建议修改版和量表级简短诊断
2. 聊天窗口只给简短摘要和 HTML 文件链接，避免输出很长的 Markdown 报告
3. HTML 默认在开头和结尾显示醒目的免责声明：AI 只能辅助发现风险，不能替代专家人工审核、回译、预测试和信效度检验
4. HTML 默认包含“检查新版本”和“复制更新提示词”，方便非技术用户确认自己是否需要更新 Skill
5. 面向初入门博士生，关键术语会用即时 hover 解释，例如 `隐性改编`、`referent`、`level of analysis`、`回译`
6. HTML 表格默认使用中文表头、P0-P3 颜色或标签、`变量/量表`、`为什么重要` 和 `谁来处理` 列
7. 只有在用户明确要求时，才额外输出 Markdown、Excel 或单独的优化版问卷对照文件
...
- 先看顶部 10 分钟决策仪表盘。
- 先处理 P0，再处理 P1。
- 先确认“研究者必须确认”的构念和层级决策，再让 RA 批量替换措辞。
- 用 `变量/量表` 列定位具体构念。
- 用 `谁来处理` 列判断是 RA 可以处理，还是必须研究者/专家确认。
