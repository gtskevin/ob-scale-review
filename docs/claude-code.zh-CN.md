...
```text
请帮我把 https://github.com/gtskevin/ob-scale-review 安装为 Claude Code 个人技能，
安装位置是 ~/.claude/skills/ob-scale-review。
安装完成后告诉我如何调用它。
```
...
Claude Code 使用技能目录名作为 slash command，所以这里是 `/ob-scale-review`。
...
- 安装时请保留整个仓库文件夹，因为 OB Scale Review 包含 references、templates、examples 和脚本。
