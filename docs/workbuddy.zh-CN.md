# 在 WorkBuddy 中使用 Survey Scale Review

[English version](workbuddy.md)

WorkBuddy 官方文档说明它是基于 Claude Code 和 MCP 的个人 agent 框架。因此，Survey Scale Review 最稳妥的安装方式是按 Claude Code Skill 路径安装。
...
```text
请帮我把 https://github.com/gtskevin/survey-scale-review 安装为本地 Agent Skill。
因为我使用的是 WorkBuddy，请优先按 Claude Code Skill 方式安装到：
~/.claude/skills/survey-scale-review

安装完成后，请告诉我如何用 /survey-scale-review 检查问卷 Excel。
```
...
## 如果没有识别到 `/survey-scale-review`

可以让 WorkBuddy 自查：

```text
请检查 ~/.claude/skills/survey-scale-review/SKILL.md 是否存在。
...
然后按其中的 Survey Scale Review 流程检查我上传的问卷。
